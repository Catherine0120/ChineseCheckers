...
void MainWindow::onTextMessageReceived(QString str) {
    ...
    if (str.left(14) == "Ready to Start") {
        ...
        ui->textEdit->append("Game Continues...");
...
void MainWindow::earlyLCDCount() {
    if (earlyLCD != 1) {
        ...
        emit lateLCDupdate();
        earlyClientRunTime++;
        if (earlyClientRunTime == 3) {
            if (goFirst == 1) {
                webSocket_1->sendTextMessage("3 RunOutOfTime!");
                webSocket_2->sendTextMessage("Enemy 3 RunOutOfTime!");
            }
            else {
                webSocket_2->sendTextMessage("3 RunOutOfTime!");
                webSocket_1->sendTextMessage("Enemy 3 RunOutOfTime!");
            }
            earlyTimer->stop();
            ui->textEdit->append("Game Ended");
            ui->textEdit->append("[Client]: 3 RunOutOfTime");
            return;
        }
        emit startLateTimer();
        earlyTimer->stop();
